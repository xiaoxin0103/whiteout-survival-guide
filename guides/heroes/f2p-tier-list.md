# 🆓 Best F2P Heroes — Complete Tier List

> **No money spent. No problem.** These are the heroes you can max without spending a dollar, ranked by value.

---

## How F2P Gets Heroes

| Method | What You Get |
|--------|-------------|
| **Lucky Wheel** | 1 mythic hero per generation. Save 160k gems. |
| **Free from Game** | Molly, Zinman, Gina, Bahiti (Gen 1 starters) |
| **Arena Shop** | Hero shards over time |
| **Events** | Occasional free hero shards |

**Your entire F2P account revolves around Lucky Wheel.** Every gem you earn should go toward Lucky Wheel heroes.

---

## F2P Lucky Wheel Priority Path

<div class="priority-flow">
<div class="priority-step">
<span class="step-badge">1</span>
<span class="step-icon">🛡️</span>
<strong>Flint</strong>
<span>Gen 2 · Infantry</span>
</div>
<div class="priority-arrow">→</div>
<div class="priority-step">
<span class="step-badge">2</span>
<span class="step-icon">⚔️</span>
<strong>Hector</strong>
<span>Gen 5 · Infantry</span>
</div>
<div class="priority-arrow">→</div>
<div class="priority-step">
<span class="step-badge">3</span>
<span class="step-icon">🔱</span>
<strong>Renee</strong>
<span>Gen 6 · Lancer</span>
</div>
<div class="priority-arrow">→</div>
<div class="priority-step current">
<span class="step-badge">4</span>
<span class="step-icon">🏹</span>
<strong>Blanchette</strong>
<span>Gen 10 · Marksman</span>
</div>
</div>

---

## Full F2P Tier List

### <span class="tier-badge tier-s">S</span> — Must-Build (F2P-Accessible)

| Hero | Gen | Class | How to Get | Why |
|------|-----|-------|------------|-----|
| <img src="/heroes/blanchette.png" alt="Blanchette" class="hero-img" width="28" height="28"> **Blanchette** | 10 | Marksman | Lucky Wheel | Best marksman. Current target. |
| <img src="/heroes/renee.png" alt="Renee" class="hero-img" width="28" height="28"> **Renee** | 6 | Lancer | Lucky Wheel | Best F2P lancer. |
| <img src="/heroes/hector.png" alt="Hector" class="hero-img" width="28" height="28"> **Hector** | 5 | Infantry | Lucky Wheel | Solid tank for mid-game. |
| <img src="/heroes/flint.png" alt="Flint" class="hero-img" width="28" height="28"> **Flint** | 2 | Infantry | Lucky Wheel | Best early F2P infantry. |

### <span class="tier-badge tier-a">A</span> — Build If Available

| Hero | Gen | How to Get | Notes |
|------|-----|------------|-------|
| <img src="/heroes/mia.png" alt="Mia" class="hero-img" width="28" height="28"> **Mia** | 3 | Lucky Wheel | Decent early Bear Hunt. Outclassed Gen 7+. |
| <img src="/heroes/gwen.png" alt="Gwen" class="hero-img" width="28" height="28"> **Gwen** | 5 | Free / Shards | Good single-target marksman. |
| <img src="/heroes/sonya.png" alt="Sonya" class="hero-img" width="28" height="28"> **Sonya** | 8 | Lucky Wheel | Decent if you skipped Renee. |

### <span class="tier-badge tier-b">B</span> — Temporary Use Only

| Hero | Why B |
|------|-------|
| Molly | Free starter. Useful for 2-3 weeks. |
| <img src="/heroes/zinman.png" alt="Zinman" class="hero-img" width="28" height="28"> Zinman | Building speed hero. Zero combat value after Gen 3. |
| <img src="/heroes/gina.png" alt="Gina" class="hero-img" width="28" height="28"> Gina | F2P starter. Replace within 1 week. |
| Bahiti | Same as Gina. |

---

## The F2P Gem Math

| Metric | Number |
|--------|:---:|
| Monthly F2P gems | ~13,000 |
| Gems needed per Lucky Wheel hero | 160,000 |
| **Months to save per hero** | **~12** |

<div class="callout danger">
<div class="callout-title">This Is Why You Can't Waste Gems</div>
At 13,000 gems/month as F2P, a single speedup purchase of 1,000 gems costs you <strong>2.3 days</strong> of progress toward your next Lucky Wheel hero. Every gem counts.
</div>

---

## F2P Timeline (Optimal)

| Account Age | What You Should Have |
|-------------|---------------------|
| Month 1 | HQ ~18, VIP 6, saving gems |
| Month 3 | First Lucky Wheel hero (Flint or skip to Hector) |
| Month 6 | Two Lucky Wheel heroes, HQ ~24 |
| Month 12 | Three Lucky Wheel heroes, HQ ~28 |
| Month 18 | Full F2P team: Flint + Renee + Blanchette, HQ 30 |

---

> **Spending $5/month?** See the [Gem Spending Guide →](/guides/beginners/gem-spending) — the monthly gem pack cuts your hero saving time in half.
